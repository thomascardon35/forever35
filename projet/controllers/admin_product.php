...
if ($login == "1"){ // Si le visiteur s'est identifié,on affiche la page
    ...
    if(array_key_exists('addProduct', $_POST) || array_key_exists('updateProduct', $_POST)){
        ...
        // ^pour $_POST['ref'] = pas besoin de intval puisque dans le try on utilise ctype_digit pour faire la vérification
        $ref                = $_POST['ref'];  
    ...
    if(array_key_exists('update_product', $_GET)){
        $productUpdate = $productModel->readById(intval($_GET['update_product']));
    }

    if(array_key_exists('delete_id', $_POST)){ 

        $productModel->delete(intval($_POST['id'])); 
